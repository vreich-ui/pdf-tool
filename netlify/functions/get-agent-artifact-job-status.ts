...
import { getAgentArtifactJobStatus } from "../lib/agent-artifact-mcp.js";
...
export async function handler(event: FunctionEvent) {
  ...
  const result = await runWithStorageGrant(__grant.grant, () => getAgentArtifactJobStatus(input(event) as { projectId: string; jobId: string }));
