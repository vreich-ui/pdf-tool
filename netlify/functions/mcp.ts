...
const TOOL_METADATA: ToolMetadata[] = [
  ...
  },
  {
    ...
    description: "T2/T3: read-only, deterministic layout analysis of an image that is ALREADY in the tenant's store — the call you make BEFORE annotate_image to find out where text can go. Returns discrete, named choices rather than raw pixels: a fixed 6x6 grid (cells \"A1\"..\"F6\", columns A-F left-to-right by rows 1-6 top-to-bottom — the SAME cell ids an AnnotationSpec's `at` field takes) with each cell's mean relative luminance (`lum`, 0=black..1=white), Sobel edge density (`busy`, 0=flat..1=maximally detailed) and mean color; up to 5 ranked `safeZones`; and a small `dominant` color palette. SAFE ZONES, SCORE SCALE DOCUMENTED (fixed 2026-09-16 — see docs/KNOWN_ISSUES.md): each is a normalized {x,y,w,h} rectangle plus a `score` in [0,1] = area (fraction of the canvas, 0-1) x quietness (1 - mean busyness, 0-1) x contrast headroom (how far the zone's mean luminance sits from middle grey, 0=none..1=black-or-white extreme) — higher is better, sorted best-first. A returned zone is ALWAYS a genuine quiet region: every candidate is screened against a busyness ceiling BEFORE scoring (both the zone's mean busyness and its single busiest cell — a large rectangle can no longer dilute one busy patch into a passing average) and a luminance-consistency check (a zone can't average a bright area against a dark one and still call itself \"quiet\" — there has to be one consistent background), and the whole canvas is NEVER a candidate at all, so \"use the top safe zone\" can never resolve to \"use the whole image\". The returned list is also nesting-free: no two zones have one (near-)fully contained inside the other. No ML, no randomness: the same bytes always produce deep-equal output (fixed thresholds baked in — see analyze.ts's SAFE_ZONE_MAX_MEAN_BUSY / SAFE_ZONE_MAX_CELL_BUSY / SAFE_ZONE_MAX_LUM_STDDEV / SAFE_ZONE_CONTAINMENT_DEDUP_THRESHOLD, all exported for pinning). `faces` is always [] and `subject` always null — detection is a later phase, and they are stable placeholders rather than a promise. Reads bytes in-function, writes NOTHING, and returns no bytes of any kind. ACCESS is scoped IDENTICALLY to verify_agent_artifact — same projectId + requestId + (artifactReference | blobKey/sha256) (+ optional materializationProof); an out-of-scope reference is refused with ARTIFACT_NOT_VERIFIED. Refuses a non-image artifact with ANNOTATE_ARTIFACT_NOT_IMAGE and an undecodable one with IMAGE_DECODE_ERROR.",
    annotations: { readOnlyHint: true, openWorldHint: false },
