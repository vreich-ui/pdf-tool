import { issueAuthorizationCode, isAllowedRedirectUri, oauthOwnerSecret, OAUTH_SCOPE, verifyOwnerSecret } from "../lib/mcp-oauth.js";
...
export async function handler(event: FunctionEvent) {
  ...
  if (event.httpMethod === "POST") {
    ...
    // Codes are self-contained signed envelopes — issuing one needs no storage, so the
    // consent step never depends on Blobs.
