 ...
 *
 * This module (and therefore @react-pdf/renderer, react, and sharp) is reachable ONLY via
 * pdf-render/render-registry.ts, imported only by pdf-render/render.ts, imported only by the
 * worker functions — never by mcp.ts. See react-pdf.ts for the lightweight metadata half that
 * mcp.ts-reachable code actually uses.
 */
