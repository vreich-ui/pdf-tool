...
export function buildArtifactJobRequestSchema() {
  return z.object({
    ...
    lenient: z.boolean().optional()
      .describe("PDF jobs only: opt this job OUT of the default strict data binding (a template variable/path the job's data omits normally fails the render with DATA_BINDING_ERROR). Strict binding applies to OUTPUT positions only: a slot the template merely TESTS — `{% if %}`/`{% unless %}`/`{% case %}`, a `{% for %}` over a per-row collection, or a react-pdf `$if` — is OPTIONAL, exactly as derive_render_data_schema types it, and is materialized before the render (an array as `[]`, anything else as `null`) rather than failing. lenient:true restores the old permissive behaviour — missing data of ANY kind renders as empty output plus an engineWarnings entry — for callers that genuinely want a best-effort render of incomplete data."),
