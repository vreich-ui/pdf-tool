import { appendArtifactJobToWorkflow } from "./agent-artifact-workflow-records.js";
...
export interface GetAgentArtifactBySlotInput { requestId: string; slot: string }
export interface GetAgentArtifactByFilenameInput { requestId: string; filename: string }
...
export async function createAgentArtifactJob(input: CreateAgentArtifactJobInput, options: { baseUrl?: string; token?: string } = {}) {
  ...
  await appendArtifactJobToWorkflow(job);
  try {
...
export async function getAgentArtifactJobStatus(input: GetAgentArtifactJobStatusInput) {
  ...
  return { ok: true as const, statusCode: 200, jobId: job.jobId, projectId: job.projectId, requestId: job.requestId, artifactKind: job.artifactKind, status: job.status, artifact: job.artifact, error: job.error };
}


export async function getAgentArtifactBySlot(input: GetAgentArtifactBySlotInput) {
  if (!input.requestId || !input.slot) return { ok: false as const, statusCode: 400, error: "requestId and slot are required" };
  if (!isSafeOptionalPathSegment(input.slot)) return { ok: false as const, statusCode: 400, error: "slot must be a safe path segment" };
  const artifact = await readArtifactReferenceBySlot(input.requestId, input.slot);
...
export async function getAgentArtifactByFilename(input: GetAgentArtifactByFilenameInput) {
  if (!input.requestId || !input.filename) return { ok: false as const, statusCode: 400, error: "requestId and filename are required" };
  const artifact = await readArtifactReferenceByFilename(input.requestId, input.filename);
  if (!artifact) return { ok: false as const, statusCode: 404, error: "Artifact not found" };
  return { ok: true as const, statusCode: 200, artifact };
}
