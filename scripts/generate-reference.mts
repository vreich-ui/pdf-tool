...
const FUNCTION_SEMANTICS: Record<string, FunctionSemantics> = {
  ...
  "export-capture-screenshots.ts": { methods: ["GET", "POST"], auth: "Bearer AGENT_RUN_TOKEN", grant: "optional and IGNORED (capture reads pdf-tool's own store)", purpose: "W2.1/G6: exports a capture job's own `[\"capture\",\"screenshot\"]` artifact BYTES (base64) by snapshot path, so the platform CI fidelity job can score the source side of each pair. Deliberately NOT an MCP tool \u2014 pdf-tool's MCP surface stays metadata-only.", status: "CURRENT", handler: "exportCaptureScreenshots" },
