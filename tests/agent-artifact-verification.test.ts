...
test("attestation alone proves materialization with no storage access", async () => {
  const { reference, proof } = await materialize("req-verify-attest");
  // Drop every storage credential: no grant, no client env. Only the signed proof remains.
