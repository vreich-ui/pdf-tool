...
| `MEASURED_BOX_DRIFT` | renderer, after the real Chromium render | the element's **rendered** box differs materially from the box **predicted** offline — the text-measurement heuristic (`defaultMeasureText`, ~0.52em/char) was wrong for this string (all-caps and all-wide-glyph text are the known bad case: see KI-30) | if the drift is on the height axis, the element likely now overlaps whatever is below it — re-check the render visually; there is no automatic re-layout |
