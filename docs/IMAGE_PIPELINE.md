...
flowchart LR
  ...
  R -- omitted --> POL[image-model-policy.json by usageContext<br/>article_header/body/category_page → fal-ai/flux-2/klein/9b<br/>else descriptor.defaultModel → AGENT_ARTIFACT_DEFAULT_MODEL → fal-ai/flux-2/klein/9b]
...
Computed once from a 384×384 stretched working buffer (`WORKING_SIZE`), by Sobel edge density (`busy`) and mean luminance (`lum`) per cell. `safeZones`'s `score` is in `[0, 1]`: `area` (fraction of the canvas, 0-1) `x` `quietness` (`1 - meanBusy`, 0-1) `x` `contrastHeadroom` (how far the zone's mean luminance sits from middle grey 0.5, 0-1) — higher is better, sorted best-first. **Fixed 2026-09-16 (KI-42):** a candidate is screened BEFORE scoring against a busyness ceiling (both its mean busyness, `SAFE_ZONE_MAX_MEAN_BUSY`, and its single busiest cell, `SAFE_ZONE_MAX_CELL_BUSY`, so a busy patch can't hide by being diluted into a large rectangle's average) and a luminance-consistency check (`SAFE_ZONE_MAX_LUM_STDDEV`, so a zone can't average a bright region against a dark one and call itself "quiet"); the whole canvas is NEVER a candidate; and de-dup now checks containment as well as IoU (`SAFE_ZONE_CONTAINMENT_DEDUP_THRESHOLD`), so the returned zones never nest one inside another. Before this fix, `safeZones` routinely ranked the entire canvas `{0,0,1,1}` #1 with the remaining slots filled by nested supersets/subsets of it — see `docs/KNOWN_ISSUES.md` KI-42 for the live repro numbers. Deterministic for a **pinned sharp/libvips build** — same caveat as the PNG golden (BRIEF §3 finding 3, `docs/KNOWN_ISSUES.md` KI-36): the `fit:"fill"` resize and the Sobel pass both run through libvips, whose resize kernel can change across sharp/libvips releases, so "the same bytes always produce the same `LayoutHints`" holds run-to-run on one pinned dependency version, not as an absolute guarantee across an npm upgrade.
...
| `CONTRAST_LOW` | resolver, via `buildLuminanceSampler` | this element's own text color fails the WCAG 2.1 threshold (default 4.5:1) against the SURFACE behind it — a filled `box` painted under the text is composited over the photo across the chip's real rendered bounds (`detail.backingCoverage` is how much of the text box it covers), the bare photo where no chip covers it; an auto-scrim was inserted behind it | the auto-scrim usually fixes legibility, but a gradient `scrim` painted by the spec is still NOT credited (see `docs/KNOWN_ISSUES.md` KI-33) — visually confirm, don't trust the absence of this warning either |
