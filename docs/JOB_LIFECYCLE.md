...
| 4. Charge the per-request ledger (`projects/{projectId}/budget/{requestId}.json`) | `chargeGenerationBudget` (`generation-budget.ts`) | ledger (non-atomic RMW) | 429 `GENERATION_BUDGET_EXCEEDED`; **fails open** on storage error |
