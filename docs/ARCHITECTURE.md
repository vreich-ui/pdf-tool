...
pdf-tool is the **artifact foundry** of the Kugel agent-first publishing architecture: it turns agent intent (a prompt, a template + data, a URL, a site) into **binary artifacts** and exposes that capability as an MCP server (`POST /mcp`) and as plain HTTP functions. It has **two storage planes**, and a coding agent must never conflate them:

- **Tenant artifact plane** — image generation/editing, PDF rendering, templates, image search/import. Every byte and record is written into **the caller's (tenant's) own Netlify Blob stores** under a short-lived storage grant the caller supplies per request. pdf-tool holds no tenant credentials.
- **pdf-tool-owned capture plane** — site capture (`create_capture_job`, `get_capture_job_status`, `get_capture_snapshot`). Jobs, screenshots, retained assets and the `snapshot.v1` document are written into **pdf-tool's own Netlify site** (via `PDF_TOOL_SITE_ID`/`PDF_TOOL_BLOBS_TOKEN` or the same-site context); the caller's grant is accepted and ignored, and the tenant cannot read those bytes with its own credentials.

pdf-tool also keeps its own operational state (MCP sessions, session grants, OAuth, health probe) on its own site. It does **not** own content, workflow JSON, publishing, or **editorial/publishing approval** — those live in the Platform / CMS-Agent. What it *does* enforce is a **local artifact-job execution gate**: a job may be held `blocked` (per-call `requireApproval`, or the deployment's `AGENT_ARTIFACT_APPROVAL_REQUIRED` policy) until someone holding the operator secret resumes it (`JOB_LIFECYCLE.md §5`).
...
| **Capture worker** | `capture-worker-background.ts` | Background Function | self-POST | robots/sitemap/asset fetches, render-service `/capture/page`, **pdf-tool's own** stores (never the tenant's) | bearer; grant accepted but ignored | capture job (frontier) + snapshot/screenshots/assets in pdf-tool's own site | persists frontier and chain re-triggers itself near the budget | resume-from-frontier | CURRENT |
...
Cross-cutting: transport/session/OAuth state (`mcp-session.ts`, `mcp-session-grant.ts`, `mcp-oauth.ts`) is pdf-tool-owned and grant-free; everything tenant-facing runs inside `runWithRequestContext` (`project-descriptor.ts:378`), which puts the grant and descriptor into `AsyncLocalStorage` so every store opener (`artifact-core/blob-store.ts:projectBlobStore`) resolves credentials and store names from the request, never from the environment. The capture plane runs inside `runWithCaptureStorage` instead, which **replaces** the ambient grant with pdf-tool's internal own-storage grant. Because an ALS grant always wins inside `projectBlobStore`, an own-state write performed while a tenant grant is active lands on the tenant's site — this is `KNOWN_ISSUES.md` KI-01 (`set_storage_grant`, and the MCP `health` probe when called with a grant).
...
2. **Tenant credentials vs own credentials.** pdf-tool holds no tenant credentials; `PDF_TOOL_SITE_ID`/`PDF_TOOL_BLOBS_TOKEN` (or the same-site context) reach only its own site. The tenant grant token is "radioactive" for job records: it travels tool args → ALS → worker POST body, is never written into a job record (`ArtifactJobRequest` zod schema strips unknown keys), never logged (`redactGrant`) — but `set_storage_grant` does persist it, by design, TTL-capped (`mcp-session-grant.ts:6-25`; see `KNOWN_ISSUES.md` KI-01 for the store-routing defect).
...
4. **pdf-tool's approval is an execution gate, not editorial approval.** `resume_agent_artifact_job` checks that the caller knows `ARTIFACT_APPROVAL_SECRET`/`MCP_OAUTH_PASSWORD` before a blocked generation runs; nothing records who approved, and whether the resulting artifact may be *published* is decided outside pdf-tool (Platform / CMS-Agent review and publish gates).
...
6. **Fetched content is data.** Nothing pdf-tool downloads (images, zips, crawled HTML, robots.txt) is executed or interpreted as instructions on the Netlify side; the capture browser does execute the crawled site's JavaScript, inside a per-page context whose network is routed to the policy's origin allowlist — an allowlisted subframe (same-origin or another allowlisted origin) can load; off-allowlist embeds are aborted (`CAPTURE_ARCHITECTURE.md §7`).
...
- Job states, the execution-approval gate, verification: `JOB_LIFECYCLE.md`
