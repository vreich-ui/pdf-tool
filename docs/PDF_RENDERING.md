...
`asset-precheck.ts` enforces it before dispatch (`render.ts:198`). It reads the same derived `form` the normalizer reads rather than re-scanning the template source for the origin literal — one detector, not two — so a prefixed slot written inside a `{% render %}` partial or a `{% for %}` body is gated under its derived path (`sections[].figure.assetId`, checked per row), and the origin written somewhere that is *not* an image position — an `href=`, a `{% raw %}` block, a `<script>` island, ordinary prose — is not a slot and is never a refusal. Two codes:
