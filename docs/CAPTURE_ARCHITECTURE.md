...
Resumption never re-queues or re-fetches a URL it already processed (queue entries are removed after processing, `queued` remembers every URL ever enqueued); a *different* URL that redirects onto an already-captured final URL is fetched once and then discarded as `duplicate` (`capturedFinalUrls`); a job that dies mid-crawl resumes from the last persisted frontier on the next trigger. There is **no** 12-minute auto-fail for capture jobs; a job whose chain trigger failed stays `pending` until something re-triggers it (a repeat `create_capture_job` for the same `requestId` does).
...
Each `pages[]` entry (built by `render-service/src/capture.ts`, ids assigned as `<pageId>_block_NNN` / `_embed_NNN`): `blocks[]` with per-viewport `boundingBoxes`/`computedStyles` and structured extraction (lists, tables, quotes, Q&A), `embeds[]` (≤ 40; `<iframe>/<embed>/<object>` src, provider classification, geometry — the **extractor** reads only the host page's DOM attributes and never traverses or waits for frame content; whether the **browser** loads a subframe is decided by the network allowlist, see §7), `fonts[]` (≤ 60; readable `@font-face` declarations and known provider stylesheet links — metadata only, no bytes at crawl time), `assets[]` (img/video/poster/document/background-image URLs with `downloaded`, `capturable`, `notCapturableReason ∈ rights_prohibited | oversize | blocked_url | fetch_failed | worker_deadline_reached | job_asset_byte_cap_reached | null`, and on success `storedArtifact`), `navigation`, `discoveredLinks`, screenshots (full page per viewport + per block; any block screenshot failure fails the whole page capture with `CAPTURE_SCREENSHOT_FAILED`).

**Content-deterministic vs run-specific fields.** There is **no byte-determinism guarantee** for a `snapshot.v1` document or for a `pages[]` entry; consumers that compare crawls must compare field by field.

| Class | Fields | Why |
|---|---|---|
| Content-deterministic (same page, same policy, same browser image ⇒ same value) | `pages[].pageId` (sha256-derived from the URL, `capture.ts:stablePageId`), block/embed ids (`<pageId>_block_NNN` sequential), `blocks[]` text/structure, `embeds[]`, `fonts[]` (sorted deterministically), `assets[].url`, `assets[].storedArtifact {path, sha256, contentType, byteLength}` (deliberately **not** `blobKey`/`createdAtISO`), `capture.policy`, `capture.viewports` | derived from page content, not from the run |
| Run-specific | top-level `capture.capturedAt` (`worker.ts:453`); **`pages[].capturedAt`** (`render-service/src/capture.ts:1269`, one per page); `capture.robots.fetchedAt` (+ `robots.sha256`/`status` if the file changed); `pages[].status` (server response); `pages[].screenshots[].sha256`/`byteLength` (PNG bytes depend on rendering — fonts, animations, timing); `boundingBoxes`/`computedStyles` when the page renders differently; `diagnostics` (`queuedUrls`, `capturedPages`, `skipped`, `quarantined`); the job record's `evidence` and `resumeCount` | timestamps and rendering are per run |

No random ids are generated anywhere in the capture plane (no `randomUUID` in `capture.ts`/`worker.ts`); the capture **job id** is a UUID but lives in the job record, not in the snapshot.
...
| Screenshots, assets, snapshot | `artifacts` · canonical layout on **pdf-tool's own site** (screenshots kind `image`, assets and the snapshot kind `binary`), `requestId` = the job's request; index entries in the own-site `artifact-index` |
...
- Browser side: JS **enabled** in a fresh context per page; `context.route("**/*")` (`createCaptureRouteHandler`, `capture.ts:1005-1032`) aborts every request whose origin is not in the policy allowlist (navigation requests → `blockedbyclient`) and `route.continue()`s every request whose origin **is** allowlisted — **including subframe navigations**: a same-origin `<iframe>`, or one pointing at another `allowedCrawlOrigins` entry, is loaded by Chromium together with its allowlisted subresources, even though the extractor never reads inside it. Off-allowlist embed hosts (third-party widgets, video players) are never added to the allowlist, so their frames and subresources are aborted and recorded in `diagnostics.blockedRequests` (first 20 per page, `CAPTURE_MAX_BLOCKED_REQUESTS`). Non-2xx or non-`text/html` top-level navigation fails the page; https + DNS-hostname only (`CAPTURE_TEST_ALLOW_HTTP=1` relaxes this and must never be set in production).
