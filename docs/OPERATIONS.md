...
| `GENERATION_BUDGET_EXCEEDED` (429) | per-request USD ledger reached `GENERATION_BUDGET_USD_PER_REQUEST` (5) or 25 unpriced generations, **and** the grant set `limits.overBudget: "block"`. Under the default `"warn"` policy the same breach is reported as a `budget_exceeded` entry in the job's `warnings[]` instead, and the job runs | use a new `requestId`, raise the env, or set it to `0` to disable. The ledger lives at `projects/{projectId}/budget/{requestId}.json` in the tenant `pdf-tool-jobs` store. |
