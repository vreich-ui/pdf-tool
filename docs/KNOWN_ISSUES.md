...
> Evidence-backed findings at commit `60bdb98762e5c10849958dbd65beba73a0d1bb31`. **No functional code was changed by the audit PR** — every entry is either a code change to schedule or a documentation change already made here. Severity: H/M/L. Confidence: how sure the auditor is that the failure scenario is real as described.
...
|---|---|---|---|---|
| KI-01 | **H** | high | `set_storage_grant` writes its record to the tenant's site, reads it from pdf-tool's own — the feature is silently non-functional in production | code |
...
| KI-05 | — | high | README/env docs contradicted code (`PDF_TOOL_SITE_ID` "removed", 6 of 32 tools listed, capture "through the storage grant", etc.) | docs (done) |
...
- **Direction:** run `setSessionGrant` outside the request ALS (or have `jobBlobStore` bypass ALS explicitly), then decide whether to keep persisting tokens at all (KI-10). Add a test asserting the store's `siteID` for that write.
