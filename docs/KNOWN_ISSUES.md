...
| KI-30 | M | high | `image.annotate`'s offline text-measurement heuristic is only checked against reality AFTER the render, as a warning (`MEASURED_BOX_DRIFT`) — never before, never blocking | design (bundle real glyph metrics, or re-render on drift) |
...
### KI-30 — `image.annotate` text-fitting error is diagnosed, never prevented (M)

- **Evidence:** `netlify/lib/image-annotate/resolve.ts`'s `defaultMeasureText` is a fixed average-advance-width heuristic (`ADVANCE_RATIO`: 0.52em/char normal, 0.58em bold), documented as "+/-15% for ordinary sentence-case Latin text" and materially worse for all-caps/all-wide-glyph text. `render.ts`'s `compareMeasurements` (its own header, `:462-489`) measured this against real Chromium: sentence-case Latin, a wrapping paragraph and even all-narrow-glyph text came back with **0.00px** height error, while ALL-CAPS and all-wide-glyph text each wrapped to an unpredicted second line (+18.75px, +100% height at the fixture's font size). The honest error bar, in the module's own words, is "correct, or one whole extra line" — not a percentage band. `MEASURED_BOX_DRIFT` is raised only AFTER a real render, by comparing the resolver's prediction to Playwright's `getBoundingClientRect()`; nothing runs before the render to catch this.
- **Failure scenario:** a spec places an ALL-CAPS title at `maxWidth: 0.4` that the offline resolver predicts fits on one line; the real Chromium render wraps it to two lines, extending `fontSizePx * 1.25` past the predicted box height and visually overlapping whatever element sits below it. The call still returns `ok:true` — the only signal is a `MEASURED_BOX_DRIFT` warning in `render_report.warnings[]`, which a caller that doesn't read warnings (or reads only `qualityGate`-style pass/fail) never sees.
- **What was tried and rejected:** T6's own investigation (BRIEF §6) tested widening the text block's CSS `width` to the resolver's full `maxWidth` budget instead of the predicted `box.w` — on real Chromium this produced **zero** reduction in extra-wrapped lines on the ALL-CAPS case (the real glyph width outran even the full budget) and would have misplaced any non-top-left-anchored or center/right-aligned block. `render.ts`'s `textRules` is unchanged as a result.
- **Direction (not implemented):** ship the bundled fonts' real glyph-advance tables so `measureText` is exact offline (removes the error at the source, per the module's own header), or automatically re-resolve and re-render once from the measured widths when drift is detected. Both are out of scope for this branch.
